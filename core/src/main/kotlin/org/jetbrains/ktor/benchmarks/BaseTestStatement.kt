...
internal abstract class BaseTestStatement(val testClass: TestClass, val child: FrameworkMethod, val methodNotBound: MethodHandle, val description: Description, val collector: (Timer) -> Unit) : Statement() {
    final override fun evaluate() {
        ...
        val method = methodNotBound.bindTo(instance).asFixedArity()
        System.gc()

        val collect = CollectTimersStatement(testClass, instance, collector)
        val mainRun = mainStatement(collect, method, instance)
    ...
    protected abstract fun mainStatement(next: Statement, method: MethodHandle, instance: AbstractBenchmark): Statement
